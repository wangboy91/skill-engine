...
Agent 会先从已注册或扫描到的 Skill 中选择候选 `skill_id`，再根据目标 Skill 的 `input.schema.json` 抽取输入。缺少必填字段时不会运行 Skill，而是返回 `needs_input`。
...
The Agent selects a candidate `skill_id` from registered or scanned Skills, then extracts input from the target Skill's `input.schema.json`. If required fields are missing, it returns `needs_input` instead of running the Skill.
